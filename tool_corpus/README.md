...
layer) and `fingerprint_corpus/` (captured HTTP traffic), this corpus
is **upstream-sourced**: each file is transcribed from the upstream
tool definitions and `include_str!`'d into the binary so the model
receives the exact tool descriptions + input_schemas it was trained
against.
