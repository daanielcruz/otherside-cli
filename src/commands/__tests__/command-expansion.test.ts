...
import { shellQuote } from "@/engine/tools/builtins/exec.ts";
...
function tallyingConfig(tally: string): UserConfig {
  const command = `printf 'fired\\n' >> ${shellQuote(tally.split(sep).join("/"))}`;
  return {
    hooks: { userPromptExpansion: [{ matcher: "*", command }] },
  } as UserConfig;
}

beforeEach(() => {
  workspace = mkdtempSync(join(tmpdir(), "expansion's fixture-"));
