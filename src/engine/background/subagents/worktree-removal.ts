...
import {
  assessNestedRepos,
  hasNoNestedRepos,
  removeNestedWorktrees,
} from "./worktree-nested-repos.ts";
...
async function removeWorktree(
  ...
  // The parent's baseline cannot authorize deleting a nested repository's contents.
  const nestedRepos = await assessNestedRepos(path);
  if (nestedRepos.kind === "blocked") return false;
  if (!(await removeNestedWorktrees(path, nestedRepos.worktrees))) return false;
  if (
    (await isWorktreeLeaseLive(path)) ||
    !(await matchesBaseline(path, baseline)) ||
  ...
  if (
    !(await hasNoNestedRepos(quarantinePath)) ||
  ...
  if (!(await hasNoNestedRepos(quarantinePath))) {
    await restoreQuarantinedWorktree(repoRoot, quarantinePath, path, baseline);
    return false;
  }
  ...
  if (!removed.ok) {
    ...
    if (!contentUnchanged || !(await hasNoNestedRepos(quarantinePath))) {
      await restoreQuarantinedWorktree(repoRoot, quarantinePath, path, baseline);
      return false;
    }
