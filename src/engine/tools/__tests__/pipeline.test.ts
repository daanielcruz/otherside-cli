...
describe("PreToolUse hooks run before permission is resolved", () => {
  test("never asks for permission before PreToolUse hooks run, and blocks execution when a hook blocks", async () => {
    ...
    // The scenario this guards against: PreToolUse hooks run before
...
describe("removed tool calls", () => {
  test("returns a synthetic tool error when a previously announced MCP tool is gone", async () => {
    const result = await dispatch(
      { id: "removed-mcp", name: "mcp__playwright__screenshot", input: {} },
      { ...context(true), scopedToolHandlers: new Map() },
      { permission: async () => "allow", hooks: [] },
    );

    expect(result).toEqual({
      tool_use_id: "removed-mcp",
      content: "unknown tool: mcp__playwright__screenshot",
      is_error: true,
    });
  });
});
