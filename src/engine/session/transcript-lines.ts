...
export function transcriptFile(path: string): ReturnType<typeof Bun.file> {
  // Bun streams need extended-length paths on Windows even when file existence checks pass.
  return Bun.file(toNamespacedPath(path));
}

export async function readSessionLines(id: string): Promise<string[]> {
  const path = findSessionPath(id);
  if (path === null) return [];
  const file = transcriptFile(path);
