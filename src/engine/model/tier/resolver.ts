...
let credentialsMemo: CredentialsMemo = { bundle: null, mtimeMs: null };
...
/** Test-only: drop the mtime-based credentials memo so the next load re-reads disk. */
export function invalidateCredentialsMemoForTests(): void {
  credentialsMemo = { bundle: null, mtimeMs: null };
}
...
function loadCredentialsSync(): CredentialsBundle | null {
  ...
  if (credentialsMemo.mtimeMs === mtimeMs) return credentialsMemo.bundle;
  try {
    const bundle = JSON.parse(readFileSync(path, "utf8")) as CredentialsBundle;
    credentialsMemo = { bundle, mtimeMs };
    return bundle;
  } catch {
    credentialsMemo = { bundle: null, mtimeMs };
    return null;
  }
}
