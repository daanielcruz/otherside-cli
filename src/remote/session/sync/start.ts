...
import { registerEnvironment, shareSessionKey } from "@/remote/backend/api.ts";
import {
  clearAuth,
  decodeUserId,
  forceRefreshAuth,
...
import { b64uDecode, unwrapEnvBroadcast, wrapSessionKey } from "@/remote/crypto/e2ee.ts";
import { type Device, deviceFingerprint } from "@/remote/devices/device.ts";
import { listPeers, loadPeer } from "@/remote/devices/peers.ts";
...
export async function startSync(
  ...
  const bootstrapSessionOnBackend = async (): Promise<void> => {
    ...
    try {
      await registerRemoteSession(device, session, broker, userId, accessToken);
