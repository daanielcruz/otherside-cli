...
import { devicePath, ensureRemoteHome } from "@/remote/_infra/paths.ts";
import { currentUserId } from "@/remote/backend/auth.ts";
import type { Bytes } from "@/remote/crypto/e2ee.ts";
import { b64uDecode, b64uEncode, generateDeviceKeyPair } from "@/remote/crypto/e2ee.ts";
...
export function migrateDevice(fromUserId: string | null, toUserId: string): void {
  const fromKey = fromUserId ?? LEGACY_USER_KEY;
  if (fromKey === toUserId) return;
  const raw = readRawStore();
  if (raw.kind !== "v2") return;
  const entry = raw.store.devices[fromKey];
  if (!entry) return;
  const devices = { ...raw.store.devices, [toUserId]: entry };
