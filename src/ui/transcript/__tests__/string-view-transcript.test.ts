...
describe("StringViewTranscript", () => {
  ...
  describe("a pasted image chip", () => {
    ...
    it("opens the cached file", () => {
      const localPath = join(tmpdir(), "image cache", "1.png");
      const row = chipRow({ id: 1, mediaType: "image/png", localPath });
      expect(row).toContain(`\x1b]8;;${pathToFileURL(localPath).href}\x1b\\[Image #1]`);
