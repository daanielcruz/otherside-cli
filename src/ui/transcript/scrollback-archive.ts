...
export class SettledScrollbackArchive {
  ...
  takeBatch(width: number, presentation: TranscriptPresentation): ScrollbackBatch {
    ...
    const precedingEntries = this.archivedEntries;
    const newlyArchived = this.settled.slice(precedingEntries.length);
    this.archivedEntries = this.settled;
    const rows = renderSettledEntries(columns, newlyArchived, presentation, precedingEntries);
