...
export function renderSettledEntries(
  ...
  precedingEntries: readonly SettledEntry[] = [],
): string[] {
  const columns = Math.max(1, width);
  const lines: string[] = [];
  let previous: SettledEntry | undefined;
  for (const entry of entries) {
    if (!isPresented(entry, presentation)) continue;
    if (entry.kind === "skill" && previous === undefined) {
      previous = precedingEntries.findLast((preceding) => isPresented(preceding, presentation));
    }
    if (!attachesToPreviousBlock(entry, previous)) lines.push("");
...
function isPresented(entry: SettledEntry, presentation: TranscriptPresentation): boolean {
  return entry.kind !== "thinking" || entry.detailOnly !== true || presentation === "detailed";
}

function attachesToPreviousBlock(entry: SettledEntry, previous?: SettledEntry): boolean {
  if (entry.kind === "command_output") return true;
  if (entry.kind === "system") return isInterruptionMessage(entry.text);
  if (entry.kind !== "skill" || previous?.kind !== "user" || entry.skillName === undefined) {
    return false;
  }
  if ((entry.progress?.length ?? 0) === 0 && entry.text.length > 0) return false;
  const command = /^\/([^\s]+)(?:\s|$)/.exec(previous.text.trim())?.[1]?.toLowerCase();
  if (command === undefined) return false;
  return command === entry.skillName.toLowerCase() || getSkill(command)?.name === entry.skillName;
}
